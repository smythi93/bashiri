...
from run_on_tests4py import Tests4PyEventCollector
...
def evaluate_project(project: Project):
    ...
    report = sfl.sflkit_instrument(location, TEST_DIR, events="FUNCTION_ENTER")
    if report.raised:
        raise report.raised
    ...
    with open(RESULTS_PATH / f"{project.get_identifier()}_functions.json", "w") as fp:
